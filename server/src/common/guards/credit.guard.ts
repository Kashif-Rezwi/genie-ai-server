...
export const CreditRequirement = (amount: number) => SetMetadata(CREDIT_REQUIREMENT_KEY, amount);
...
export class CreditGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        const requiredCredits = this.reflector.getAllAndOverride<number>(CREDIT_REQUIREMENT_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
