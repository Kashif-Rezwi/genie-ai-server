...
export class CreditCleanupTask {
    ...
    async handleExpiredReservations() {
        ...
        for (const failedId of results.errors) {
            await this.handleFailedCleanup(failedId);
        }

        const duration = Date.now() - startTime;
        this.logger.log(
            `Cleanup completed: ${results.cleaned} cleaned, ${results.errors} errors, duration: ${duration}ms`,
        );
    }
    ...
    private async handleFailedCleanup(reservationId: string): Promise<void> {
        ...
        try {
            ...
            if (retryCount > this.MAX_CLEANUP_RETRIES) {
                ...
                this.logger.warn(
                    `Reservation ${reservationId} cleanup failed, retry count: ${retryCount}`,
                );
            }
        } catch (error) {
