...
export class MonitoringController {
    ...
    return {
      database:
        database.status === 'fulfilled'
          ? database.value
          : { status: 'error', error: database.reason },
      redis: redis.status === 'fulfilled' ? redis.value : { status: 'error', error: redis.reason },
      memory:
        memory.status === 'fulfilled' ? memory.value : { status: 'error', error: memory.reason },
      timestamp: new Date(),
    };
  }
  ...
  async getSystemStatus() {
    ...
    if (activeAlerts.some(a => a.severity === 'critical')) {
      ...
      metrics.requests.errors / metrics.requests.total > 0.05
    ) {
      // 5% error rate
      status = 'degraded';
    }
